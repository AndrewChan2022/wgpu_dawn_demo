...
@implementation MyComputerPass


- (instancetype)initWithVSPath:(NSString*)vspath fspath:(NSString*)fspath {
    self = [super initWithVSPath:vspath fspath:fspath];
    return self;
}
...
-(void)createProgramGetSlot:(NSString*)vs fs:(NSString*)fs {
    ...
    // create program
    NSString * vertexShaderPath = [[NSBundle mainBundle] pathForResource:vs
                                                                  ofType:@"glsl"];
    NSString * fragmentShaderPath = [[NSBundle mainBundle] pathForResource:fs
                                                                    ofType:@"glsl"];
...
-(void)drawWithMesh:(MyBaseMesh*)mesh renderTarget:(MyRenderTarget*)renderTarget {
    ...
    //glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, renderTarget.texture.textureID, 0);
    ...
    if (mesh.map != nil) {
        ...
        glUniform1i(_inputTextureSlot, 1);
    }
