...
+(GLuint)loadProgram:(NSString *)vertexShaderFilepath withFragmentShaderFilepath:(NSString *)fragmentShaderFilepath
{
    ...
    if (fragmentShader == 0) {
        ...
        if (logLength > 0) {
            ...
            NSString* err = [NSString stringWithFormat:@"%s", log];
            NSLog(err);
            assert(false);
            free(log);
        }
...
+(GLuint)loadProgram:(NSString*)vertexString withFragmentString:(NSString *)fragmentString {
    // Load the vertex/fragment shaders
    GLuint vertexShader = [self loadShader:GL_VERTEX_SHADER withString:vertexString];
    if (vertexShader == 0) {
        return 0;
    }
    
    GLuint fragmentShader = [self loadShader:GL_FRAGMENT_SHADER withString:fragmentString];
    if (fragmentShader == 0) {
        glDeleteShader(vertexShader);
